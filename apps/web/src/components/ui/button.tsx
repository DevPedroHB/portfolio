...
import { LoaderIcon } from "../animate-ui/icons/loader";
import { ShimmeringText } from "../animate-ui/text/shimmering";

const buttonVariants = cva(
	"group inline-flex justify-center has-data-[arrow=true]:justify-between items-center disabled:opacity-60 focus-visible:outline-hidden ring-offset-background font-medium text-sm whitespace-nowrap transition-[color,box-shadow] cursor-pointer disabled:pointer-events-none [&_svg]:shrink-0",
	{
		variants: {
			variant: {
				primary:
...
function Button({ isLoading = false, children, ...props }: IButton) {
	return (
		<ButtonRoot disabled={isLoading} {...props}>
			{isLoading ? (
				<>
					<LoaderIcon className="size-4 animate-spin" animate loop />
					<ShimmeringText text={children as string} wave />
				</>
			) : (
				children
			)}
